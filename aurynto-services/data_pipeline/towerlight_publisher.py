...
# this code establishes connection between the simulated crane/publisher and the MQTT broker running in Docker

while True:
    message_to_send = {"asset_id": ASSET_ID,
                       "asset_table_id": ASSET_TABLE_ID,
                       "timestamp": datetime.utcnow().isoformat(),
                       "Operating_hours_current": round(random.uniform(0, 200), 2),
                       "Operating_hours_Lifetime": round(random.uniform(500, 10000), 2),
                       "Light_Status": random.choice(["On", "Off"]),
                       "light_intensity": round(random.uniform(70, 100), 1),
                       "fuel_level": round(random.uniform(10, 100), 1),          # %
                        "engine_rpm": round(random.uniform(1200, 2500), 0),
                        "engine_temp": round(random.uniform(80, 110), 1),         # °C
